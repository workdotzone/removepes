...
export default async function LocationPage({
  ...
  return (
    <>
      ...
      <FaqSection
        title={`Frequently Asked Questions — Pest Control in ${location.name}`}
        subtitle={`Common questions about our pest control services in ${location.name}, Mumbai.`}
        faqs={[
          { q: `Do you provide pest control in ${location.name}?`, a: `Yes! We provide full pest control services across ${location.name}, Mumbai. ${location.description} Same-day and next-day appointments are available.` },
          { q: 'How long does a pest control treatment take?', a: 'Most standard treatments take 1–3 hours depending on property size and pest type. Termite and bedbug treatments may require additional follow-up visits for complete elimination.' },
          { q: `Are your chemicals safe for children and pets in ${location.name}?`, a: 'Yes. We use only government-approved, WHO-certified chemicals that are safe for families and pets after the recommended drying time of 2–4 hours post-treatment.' },
          { q: `Do you offer a warranty on treatments in ${location.name}?`, a: 'Yes, we provide a service warranty from 3 months to 1 year. If pests return within the warranty period, we offer free re-treatment at no additional cost.' },
          { q: `How much does pest control cost in ${location.name}?`, a: 'Pricing starts from ₹500 and varies by property size, pest type, and treatment method. Contact us for a free inspection and customized quote with no hidden charges.' },
          { q: `Do I need to vacate my home during treatment in ${location.name}?`, a: 'For most treatments, we recommend staying out for 2–4 hours. For intensive treatments like termite or bed bug control, 4–8 hours may be required. Our team will provide full pre- and post-treatment instructions.' },
          { q: `How many treatments are needed to eliminate termites in ${location.name}?`, a: 'Termite control typically requires 2–3 sessions using soil treatment, baiting systems, and wood treatment. A 1-year service warranty is included.' },
          { q: `Can pest control be done in a rented flat in ${location.name}?`, a: `Absolutely. We regularly service rented flats and housing societies across ${location.name}. We also work with building management offices for AMC and bulk bookings.` },
          { q: 'What preparation is needed before pest control?', a: 'Clear kitchen cabinets, remove food from countertops, cover drinking water, and keep children and pets away. Our technicians provide a full pre-treatment checklist at booking.' },
          { q: `Do you provide pest control for restaurants and offices in ${location.name}?`, a: `Yes. We specialize in commercial pest control for restaurants, hotels, hospitals, and warehouses in ${location.name}. FSSAI-compliant with audit documentation provided.` },
          { q: `Is a free inspection available in ${location.name}?`, a: `Yes, we offer a free pest inspection for all new customers in ${location.name}. Our expert will assess the infestation, identify entry points, and recommend the most effective and affordable treatment plan.` },
          { q: `How do I book pest control in ${location.name}?`, a: `Call +91-94203 00006, fill out our contact form, or send a WhatsApp message. We offer same-day and next-day appointments in ${location.name} with flexible scheduling.` },
        ]}
      />
